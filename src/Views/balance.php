...
<div class="report-title">
    Monthly Balance Sheet: <?php echo $year; ?> (<?php echo $startMonth; ?> to <?php echo $endMonth; ?>)
</div>

<div class="report-container">
<?php
printf("%-" . $widths['label'] . "s", "");
foreach ($labels as $l) printf(" || %" . $widths['amounts'][$l] . "s", substr($l, 5)); 
...
printf("%-" . $widths['label'] . "s", " Net:");
foreach ($labels as $l) {
    $assets = $periodicTotals[$l]['Assets'] ?? [];
    $liabs = $periodicTotals[$l]['Liabilities'] ?? [];
    $total = 0.0;
    foreach ($assets as $c => $a) { $total += $a; }
    foreach ($liabs as $c => $a) { $total += $a; }
