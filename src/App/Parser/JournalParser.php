        ...
        while (($line = fgets($handle)) !== false) {
            ...
            if ($currentTx && (strpos($line, ' ') === 0 || strpos($line, "\t") === 0)) {
                ...
                if (count($parts) === 2) {
                    ...
                    if (preg_match('/([-+]?[\d.,]+)/', $rest, $amtMatches)) {
                    ...
                    // Normalize amount logic (existing)
                    ...
                    if (preg_match('/^(.*?)\s+([-+]?[\d.,]+)\s*(\S+)?$/', $trimmed, $matches)) {
                        ...
                        $amount = (float)str_replace(',', '.', str_replace('.', '', $matches[2])); // crude fallback
                        $currency = $matches[3] ?? 'EUR';
                        $currentTx->addPosting(new Posting($account, $amount, $currency));
                    } else {
                        $currentTx->addPosting(new Posting($trimmed, 0.0, '???'));
                    }
                }
            }
        }
